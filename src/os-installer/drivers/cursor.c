...
static void cursor_init(uint32_t width, uint32_t height) {
    ...
    vga_get_char_at(_cursor_state.x, _cursor_state.y, &_cursor_state.last_char, &_cursor_state.last_attr);
}
...
static void cursor_show(void) {
    ...
    if (_cursor_state.type == CURSOR_DEFAULT) {
        ...
        vga_put_char_at(_cursor_state.x, _cursor_state.y, _cursor_state.last_char, flipped_attr);
    } else {
        uint8_t attrib = (_cursor_state.bg_color << 4) | _cursor_state.fg_color;
        vga_put_char_at(_cursor_state.x, _cursor_state.y, _cursor_state.type, attrib);
    }
...
static void cursor_hide(void) {
    if (!_cursor_state.visible) return;
    vga_put_char_at(_cursor_state.x, _cursor_state.y, _cursor_state.last_char, _cursor_state.last_attr);
    _cursor_state.visible = 0;
}
...
static void cursor_move(int32_t x, int32_t y) {
    if (_cursor_state.visible) {
        ...
        vga_get_char_at(x, y, &_cursor_state.last_char, &_cursor_state.last_attr);

        // Draw cursor at new position
        if (_cursor_state.type == CURSOR_DEFAULT) {
            uint8_t flipped_attr = swap_colors(_cursor_state.last_attr);
            vga_put_char_at(x, y, _cursor_state.last_char, flipped_attr);
        } else {
            uint8_t attrib = (_cursor_state.bg_color << 4) | _cursor_state.fg_color;
            vga_put_char_at(x, y, _cursor_state.type, attrib);
        }
    }
    _cursor_state.x = x;
    _cursor_state.y = y;
...
static void cursor_set_type(cursor_type_t type) {
    ...
    if (_cursor_state.visible) {
        if (type == CURSOR_DEFAULT) {
            ...
            uint8_t attrib = (_cursor_state.bg_color << 4) | _cursor_state.fg_color;
            vga_put_char_at(_cursor_state.x, _cursor_state.y, type, attrib);
        }
    }
}
