...
# Check if VHD is already attached and detach it
$diskpartCheckScript = @"
select vdisk file="$vhdPath"
detail vdisk
"@
$diskpartCheckPath = "$env:TEMP\diskpart_check.txt"
$diskpartCheckScript | Out-File -FilePath $diskpartCheckPath -Encoding ASCII
$diskpartCheckOutput = diskpart /s $diskpartCheckPath
Remove-Item $diskpartCheckPath -ErrorAction SilentlyContinue
if ($diskpartCheckOutput -match "is attached") {
    Write-Host "VHD is already attached. Detaching..."
    $diskpartDetachScript = @"
select vdisk file="$vhdPath"
detach vdisk
"@
    $diskpartDetachPath = "$env:TEMP\diskpart_detach.txt"
    $diskpartDetachScript | Out-File -FilePath $diskpartDetachPath -Encoding ASCII
    diskpart /s $diskpartDetachPath
    Remove-Item $diskpartDetachPath -ErrorAction SilentlyContinue
}
...
"@
    ...
    # Fallback: Look for FAT32 volume of ~98 MB if OSDISK not found
    if (-not $volumeNumber) {
        Write-Warning "Could not find OSDISK volume. Checking for FAT32 volumes..."
        foreach ($line in $diskpartOutput) {
            $line = $line.Trim()
            if ($line -match "FAT32\s+Partition\s+98 MB") {
                if ($line -match "Volume\s+(\d+)") {
                    $volumeNumber = $matches[1]
                    Write-Host "Found FAT32 volume (98 MB): Volume $volumeNumber"
                    break
                }
            }
        }
    ...
    if (-not $volumeNumber) {
        Write-Error "No OSDISK or suitable FAT32 volume found. Check diskpart output above."
        exit 1
    }
